...
**Location**: `supabase/migrations` に DB 変更が保存される。汎用的な `infra/` ディレクトリは現時点で未作成。環境変数は各パッケージの `.env.example` で共有予定。ルートディレクトリの `verify_scenario.py` は、PDF アップロードからチャット応答までの一連のシナリオを検証するスクリプト。
